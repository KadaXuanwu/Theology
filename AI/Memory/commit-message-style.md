...
description: Commit messages in the Theology repo stay one line, under 72 characters, with no Co-Authored-By trailer for the agent.
metadata:
  type: feedback
---

Write commit messages as a single line **of 72 characters or fewer**. Add a body only when the change genuinely needs one. Never append a `Co-Authored-By` trailer naming the agent.

**Why the trailer is banned:** it made GitHub render the commit as authored by the user and an account named for the agent, which the user does not want on this repo. Long commit bodies are noise for a notes vault where most commits are content edits.
