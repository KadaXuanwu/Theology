...
Links never run up the stack. An argument may link claims, evidence, people and other arguments, except in its Description, which may not link another argument. A claim may link other claims, evidence and people, never an argument. An evidence note may link other evidence and people, nothing above it. A person node links only other people. Any node may link a term in Glossary, on the first use of the term, and a term links only other terms. The upward direction never gets written by hand. The site works it out and shows it under "Linked from" on every note. `npm run check` lints all of this, along with the template headings, dashes, footnotes and dead source URLs; run it before you push.
...
A node's `status` is a promise. `stub` is my own notes, written by hand. `drafted` is the idea in the node's shape, unverified, made by the `draft-node` skill. `sourced` has passed the verifier, made by `source-node`, and means the node can be shown to the public with confidence. `sourced-stale` passed once and is due again: an older pipeline built it, or something in it is wrong. `refresh-node` re-earns `sourced` at the standard of a full run, spending the research on what the node lacks or gets wrong. A stub or a draft is inspiration for the sourcing run and nothing more. The one thing in it that binds is the `points` list in the frontmatter, which is mine: carry every point with a source or tell me why not, and never edit the list. The gates are in `.agents/skills/source-node/references/templates.md`.

Three skills build the nodes and live in `.agents/skills/`. `draft-node` takes a stub to drafted, `source-node` takes a draft to sourced, and `refresh-node` takes a sourced-stale node back to sourced. If your agent scans that folder they trigger on their own. If it does not, open the matching `SKILL.md` and follow it when I ask to draft, source or refresh a node. Notes on both are in AI/Skills/README.md.
