...
Two skills build the nodes and live in `.agents/skills/`. `draft-node` takes a stub to drafted and `source-node` takes a draft to sourced. If your agent scans that folder they trigger on their own. If it does not, open the matching `SKILL.md` and follow it when I ask to draft or source a node. Notes on both are in AI/Skills/README.md.
